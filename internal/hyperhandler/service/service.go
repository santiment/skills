...
// WalletAndSigner resolves the private key for network and builds a signer.
// Returns a nil signer (no error) when no key is configured, so callers can
// surface the "no private key" guidance.
