...
// (env var, system keyring, HD mnemonic) and exposes key utilities. Key entry is
// non-interactive: keys come from the environment or the OS keyring only.
package wallet
...
// (env → keyring). Key entry is non-interactive.
type WalletManager struct { //nolint:revive // established public name across the wallet API
	providers []KeyProvider
	keyring   *KeyringProvider
}

// NewWalletManager builds the default non-interactive provider chain
// (env → keyring).
