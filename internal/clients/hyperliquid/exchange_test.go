...
func TestPlaceOrderFilled(t *testing.T) {
	...
	ex := hyperliquid.NewExchangeClient(netFor(srv.URL), testSigner(t), slippage)
...
func TestCancelOrderServerErrorReturnsFalse(t *testing.T) {
	...
	ex := hyperliquid.NewExchangeClient(netFor(srv.URL), testSigner(t), slippage, fastRetry(), hyperliquid.WithMaxRetries(0))
