...
import (
	...
	"santiment.net/san-skills/internal/hyperhandler/order"
...
// returns the number canceled, 0 on any error.
func (c *ExchangeClient) CancelAllOrders(ctx context.Context, vaultAddress *string) int {
	...
	var resp struct {
		...
		Response struct {
			Data struct {
				Canceled int `json:"cancelled"` //nolint:misspell // wire key is British "cancelled"
			} `json:"data"`
		} `json:"response"`
	}
	...
	return resp.Response.Data.Canceled
}
