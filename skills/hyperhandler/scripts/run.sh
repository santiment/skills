#!/usr/bin/env bash
# Self-contained launcher for the `hyperhandler` CLI bundled inside this skill.
#
# The binary ships next to this script as a gzip+base64 text blob
# (hyperhandler-<os>-<arch>.gz.b64) so it survives `npx skills add` transport
...
#
# Usage: bash scripts/run.sh <hyperhandler args...>   (e.g. describe --json)
set -euo pipefail
...
bin="$here/.bin/hyperhandler-$os-$arch"
src="$here/hyperhandler-$os-$arch.gz.b64"

if [ ! -f "$src" ]; then
  echo "hyperhandler: no bundled binary for $os/$arch" >&2
  echo "hyperhandler: rebuild it from the repo with: task skill-bundle (add $os/$arch to SKILL_PLATFORMS)" >&2
  exit 7
fi
